...
class __infix_to_prefix
{
  ...
  private:
    ...
    {
        std::string str;
        while (ind < _expr.size() &&
               (is_digit_(_expr[ind]) || is_alpha_(_expr[ind]))) {
            str += _expr[ind++];
        }
        return str;
    }
    ...
    bool cmp_priority_(const operand& lhs, const operand& rhs)
    {
    ...
    std::vector<operand> convert()
    {
        for (size_t ind = 0; ind < _expr.size();) {
            if (is_digit_(_expr[ind])) {
                ...
                _func.pop_back();
                if (!_func.empty() && !is_open_bracket_() &&
                    _func.back().operands_number == 0) {
                    pull_to_prefix_();
                    if (_expr[ind - 1] != '(') {
                        _prefix.back().operands_number =
                            _comma_counter.back() + 1;
                    }
...
class __jit_compiler
{
  ...
  private:
    ...
    {
        for (size_t i = 0; externs[i].name != 0 || externs[i].pointer != 0;
             ++i) {
            std::string name;
